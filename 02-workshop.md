...
```mermaid
flowchart TD
    A["🗄️ <b>Step 1: โครงสร้างฐานข้อมูล & Seeders</b><br>ตารางและข้อมูลตั้งต้นบัญชีทดสอบ"] --> B["👑 <b>Step 2: [เมนูที่ 1] ระบบจัดการห้องประชุม (Admin)</b><br>Backend CRUD + หน้า React จัดการห้องประชุม"]
    B --> C["👥 <b>Step 3: [เมนูที่ 2] ระบบจัดการผู้ใช้ (Admin)</b><br>Backend CRUD + หน้า React จัดการผู้ใช้งานและบทบาท"]
    C --> D["🎛️ <b>Step 4: [หัวใจหลัก] บริการตรวจจองซ้อน (Service)</b><br>เขียนคลาสเช็กเวลาจองทับซ้อนและข้อมูลความจุ"]
    D --> E["👨‍🏫 <b>Step 5: [เมนูที่ 3] ระบบค้นหาและยื่นจอง (Teacher)</b><br>หน้าจอรรายการห้อง / ดูคิวล่วงหน้า / ฟอร์มขอจอง / ดูประวัติ"]
    E --> F["👮 <b>Step 6: [เมนูที่ 4] ระบบพิจารณาอนุมัติ (Staff)</b><br>หน้าตรวจสอบคำขอ รออนุมัติ และระบุเหตุผลปฏิเสธ"]
```
...
## 2. ขั้นตอนปฏิบัติการ Workshop (6 Steps - CRUD ทีละเมนู)
...
### 🗄️ Step 1: โครงสร้างฐานข้อมูลและบัญชีทดสอบเริ่มต้น (Database & Seeders)
* **เป้าหมาย:** สร้างตารางข้อมูลหลักและเซ็ตข้อมูลบัญชีทดสอบ 3 บทบาท รวมถึงเพิ่มห้องประชุมเริ่มต้น เพื่อให้ระบบมีข้อมูลพล็อตในหน้าจอทันที
* **⚡ Short Prompt:**
  ```text
  สร้าง migrations และ models ของระบบจองห้องประชุม: users (เพิ่ม role enum [teacher, staff, admin], department, phone, soft deletes), rooms, และ bookings จากนั้นสร้าง UserSeeder (บัญชีบทบาทละ 1 คน รหัสผ่าน password) และ RoomSeeder (สร้าง 5 ห้องตัวอย่าง) ผูกเข้า DatabaseSeeder แล้วสั่งรัน migrate และ db:seed
  ```
...
### 👑 Step 2: [เมนูที่ 1] ระบบจัดการข้อมูลห้องประชุมสำหรับแอดมิน (Admin's Rooms CRUD)
...
### 👥 Step 3: [เมนูที่ 2] ระบบจัดการข้อมูลผู้ใช้งานสำหรับแอดมิน (Admin's Users CRUD)
...
### 🎛️ Step 4: [บริการหลัก] บริการตรวจเช็คระบบคิวจองทับซ้อน (Booking Overlap Service)
...
### 👨‍🏫 Step 5: [เมนูที่ 3] ระบบค้นหาห้องและยื่นใบจองสำหรับผู้ใช้ (Teacher's Booking Flow)
* **เป้าหมาย:** ระบบสืบค้นข้อมูลและจองห้องสำหรับคุณครู (Teacher) โดยจะเรียกใช้งาน BookingService ช่วยกรองข้อมูลก่อนบันทึก พร้อมหน้าจอ React แบบแสดงประวัติและการกดยกเลิกคิวจอง
* **⚡ Short Prompt:**
  ```text
  สร้าง RoomController (ดึงหน้าการ์ดห้อง Rooms/Index และรายละเอียดห้องพร้อมคิวจองล่วงหน้า Rooms/Show), BookingController (ให้ผู้ใช้ทำเรื่องจองผ่าน BookingService และจัดการยกเลิก), กำหนดสิทธิ์และ routes, และสร้างหน้า React: Rooms/Index.jsx (รายการห้อง), Rooms/Show.jsx (รายละเอียดและปฏิทินจอง), Bookings/Create.jsx (ฟอร์มทำเรื่องจอง), และ Bookings/Index.jsx (ประวัติคิวจองของผู้ใช้)
  ```
* **🔍 วิธีตรวจสอบ:** ล็อกอินเข้าบัญชี `teacher@example.com` ลองกดเลือกห้องเพื่อขอยื่นจองห้องตามวันเวลาต่างๆ และตรวจสอบผลลัพธ์ (เช่น หากระบุเวลาซ้ำกันกับคิวจองล่วงหน้า ระบบต้องบล็อกและแจ้งข้อผิดพลาดเป็นตัวอักษรสีแดงเตือนทันที)

---

### 👮 Step 6: [เมนูที่ 4] ระบบตรวจสอบและพิจารณาอนุมัติคิวจอง (Staff's Approvals)
...
* **⚡ Short Prompt:**
  ...
  ```
* **🔍 วิธีตรวจสอบ:** ล็อกอินด้วยบัญชี `staff@example.com` เข้าเมนูอนุมัติการจอง ลองกดปุ่มอนุมัติ หรือระบุเหตุผลเพื่อปฏิเสธคำขอ จากนั้นสลับกลับไปเช็กสถานะการเปลี่ยนแปลงในบัญชีของคุณครู (Teacher) ว่ารายการได้รับการอัปเดตตรงตามสิทธิ์แล้วหรือไม่
